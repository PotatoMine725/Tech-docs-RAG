...
| `09:header-1600:0001` | C# language reference > C# language reference | 24 | **Heading-only chunk**: `## C# language reference`. The H2 is followed directly by H3 children, and D3 merges a small section only with its next *sibling*, so nothing merges into it. 19 such chunks in Arm A (`heading_only_chunks` in the stats). Retrieval noise candidate; changing it needs an ADR-0003 amendment (see report, open items). **INGEST-004 (ADR-0003 D3a):** this chunk is dropped; the ID `09:header-1600:0001` now names the former `0002`. |
