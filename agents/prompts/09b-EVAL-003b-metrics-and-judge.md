...
- **Strict and lenient (owner, 2026-09-25, OWNER-001):** report `source_hit@k` and `section_hit@k` both **lenient** (the rule above: expected + alternate sources/sections per slot) and **strict** (expected sources/sections only; alternates ignored). Tag every span in `expected-spans-v1.json` with its role (`expected` / `alternate`) so strict section hit can be computed. MRR and the slot fraction use the lenient rule only. Which value is the headline: TBD / DECISION REQUIRED (`evaluation-spec.md` § Retrieval hit rule).
