...
```text
Execute agents/prompts/06a-RAG-001a-embedder-and-cache.md.
Entry is met: eval-freeze-v1 tag exists (739676f, verified by the owner's consultant), INGEST-004 verified.

Git (do it yourself with git + gh; never force-push, never touch main):
- Start: git fetch origin; branch rag-001a from origin/dev; confirm a clean tree.
- Commit per milestone; push after each; tests green at every push.
- End: gh pr create --base dev --head rag-001a. Do not merge (the owner runs 99-VERIFY first).

The owner is at the machine for the V-1 probe: tell me exactly when to read the AI Studio usage page (before and after), and wait.
Live calls: only the V-1 probe (≤ 3 texts) and the one @pytest.mark.gemini embed test. No indexing in this task.
```

## Task prompt (`agents/prompts/06a-RAG-001a-embedder-and-cache.md`, verbatim)
