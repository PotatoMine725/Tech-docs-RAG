# INGEST-002 — prompt log

## Owner message that started the session (2026-09-25)

```
Read docs/plans/session-handoff-2026-09-25.md, then continue with its "Next prompts" list.
```

Next prompt 1 (owner verdicts) was still open and 2 (EVAL-001 re-verify) needs a fresh session, so the session ran next prompt 3: `agents/prompts/04-INGEST-002-chunkers-and-stats.md`, saved verbatim below.

## Owner answer during the task (AskUserQuestion)

Q: Tables over 1,600 chars: header row repeated in `embed_text` only, or in `display_text` too (breaking the G2 slice rule)?
A: "Header in embed_text only (Recommended)".
