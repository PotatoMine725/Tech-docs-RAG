...
- DOCUMENT CONVERSION: Microsoft MarkItDown `markitdown[pdf,docx]>=0.1.8,<0.2` (ADR-0002; added in INGEST-003; only `infrastructure/parsing/markitdown_parser.py`). Pulls in `magika` → `onnxruntime` as a transitive dependency of MarkItDown itself (not an ML feature of this app; imported lazily).
