...
| 05 | `05-INGEST-003-markitdown-optional.md` | INGEST-003 | 03 | done | "INGEST-003: MarkItDown adapter for PDF/HTML/DOCX/txt, V-2, OD-6" (branch `claude/workflows-ingest-002-review-svn2dq`) | [INGEST-003](../reports/execution/INGEST-003.md) | — (awaiting `99-VERIFY`) | Run anyway (not behind schedule, OD-15 cut not needed). Owner to review AI decisions: OD-6 (one adapter, stubs deleted), converted files skip the D1 normalizer, transitive `onnxruntime`. V-2 checked on Linux 3.13.12 only; Windows venv needs `pip install -r requirements.txt`. |
