...
| 03 | `03-INGEST-001-models-parser-normalization.md` | INGEST-001 | 00 + 00R verified (parallel with 01/02) | verified with fixes | `1858e27`; "INGEST-001: fixes from verification" (not yet re-verified) | [INGEST-001](../reports/execution/INGEST-001.md) | [INGEST-001-verify](../reviews/code/INGEST-001-verify.md) → ACCEPT WITH FIXES | Docs-only fixes 1–4 in the review's fix prompt (boilerplate list in one place + its sources; footer removal recorded as an extension of ADR-0003 D1; worklog/report provenance wording; GitNexus compare-scope result), then re-verify. INGEST-002 stays blocked until this row is `verified`. Not evidenced: mutation count, pre-edit impact runs, chat "Explain it back". |
